...
def build_mlp(
    ...
    #========================================================================================#
    x = tf.layers.dense(inputs=input_placeholder, units=size, activation=activation)
    with tf.variable_scope(scope):
        for i in range(n_layers-1):
            x = tf.layers.dense(inputs=x, units=size, activation=activation)
        output = tf.layers.dense(inputs=x, units=output_size, activation=output_activation)
        return output
...
def train_PG(exp_name='',
    ...
    if discrete:
        ...
        sy_sampled_ac = sy_mean + tf.multiply(tf.exp(sy_logstd), tf.random_normal(tf.shape(ac_dim)))
    ...
    weighted_loss = tf.multiply(sy_logprob_n, sy_adv_n)
    ...
    if nn_baseline:
        ...
        # YOUR_CODE_HERE
        baseline_target = tf.placeholder(shape=[None], name="baseline", dtype=tf.float32)
    ...
    for itr in range(n_iter):
        ...
        # YOUR_CODE_HERE
        if reward_to_go:
            q_n = np.concatenate([[np.sum([path['reward'][td]*gamma**(td - t) for td in range(t,pathlength(path))]) for t in range(pathlength(path))] for path in paths])
        else:
            print(pathlength(path))
            q_n = np.concatenate([[np.sum([gamma**i*path['reward'][i]  for i in range(pathlength(path))])] * pathlength(path) for path in paths])
